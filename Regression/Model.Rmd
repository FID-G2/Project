...
Es importante destacar que el error que devuelven los modelos se encuentra entre 0 y 1 porque la columna de Nota media fue escalada.
...
**Aclaración**: No se utiliza la búsqueda por rejilla de caret debido a que en varios experimentos hemos sufrido interrupciones de la ejecución debido a limitaciones de memoria RAM de nuestros equipos.
...
Dado que la nota media se escaló es necesario multiplicar el resultado de las predicciones por la mayor nota media que aparecía en el dataset.

```{r}

datos_tratados = read.csv('../data/CleanData/datos_tratados.csv')
max_nota_media <- max(datos$Nota.media)

print(max_nota_media)
```
...
print(predicciones[1]*max_nota_media)
```
