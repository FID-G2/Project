...
### 3.2. Input del usuario

```{r}
nuevos_datos_rf <- data.frame(
    Comunidad.Autónoma = "Comunidad.AutónomaAndalucía",
    Sexo = "Hombres",
    Forma.de.admisión = "PAU",
    Rama.de.enseñanza = "Rama.de.enseñanzaCiencias.Sociales.y.Jurídicas",
    Ámbito.de.estudio = "Ámbito.de.estudioEconomía",
    Zona.de.nacionalidad = "Zona.de.nacionalidadEspaña"
)
```
...
df_genero <- binary_encode(nuevos_datos_rf, "Sexo", "Hombres", "Mujeres")
...
agregar_pca_a_datos <- function(data, df_indices, nombre_columna) {
  ...
  if (nrow(fila) > 0) {
    ...
    # Renombrar las columnas a Comunidad.PCA1, Comunidad.PCA2, ...
    colnames(valores_pca) <- paste0("Comunidad.", colnames(valores_pca))
...
# Uso de la función
agragar_pca_comunidad <- agregar_pca_a_datos(df_nivel, data, "Comunidad.Autónoma")
head(nuevos_datos_con_pca)
```
