...
predicciones <- predict(model_rf_alternative, newdata = datos_prueba)
...
# Realizar predicciones en el conjunto de prueba
predicciones_prueba <- predict(modelo_lgb, data_prueba)

# Evaluar el rendimiento del modelo en el conjunto de prueba
metricas_prueba <- list(
  RMSE = sqrt(mean((predicciones_prueba - label_prueba)^2)),
  MAE = mean(abs(predicciones_prueba - label_prueba)),
  MSE = mean((predicciones_prueba - label_prueba)^2)
)
...
modelo_tree <- train(Nota.media ~ ., data = datos_entrenamiento, method = "rpart", trControl = control, control = rpart.control(cp = 1))

# Mostrar el modelo entrenado
print(modelo_tree)

# Realizar predicciones en el conjunto de prueba
predicciones_prueba <- predict(modelo_tree, newdata = datos_prueba)

# Evaluar el rendimiento del modelo en el conjunto de prueba
metricas_prueba <- list(
  RMSE = sqrt(mean((predicciones_prueba - datos_prueba$Nota.media)^2)),
  MAE = mean(abs(predicciones_prueba - datos_prueba$Nota.media))
)
