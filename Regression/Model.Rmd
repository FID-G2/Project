...
# Entrenar el modelo Random Forest con el 80% de los datos de entrenamiento

```{r}
model_rf_alternative <- train(Nota.media ~ ., data = datos_entrenamiento, method = "rf", trControl = control, tuneGrid = data.frame(mtry = 2), ntree = 10)


print(model_rf_alternative)
...
metricas <- postResample(predicciones, datos_prueba$Nota.media)
print(metricas)
```
...
model_tree_alternative <- train(Nota.media ~ ., data = datos_entrenamiento, method = "rpart", trControl = control, control = rpart.control(cp = 1))

# Mostrar el modelo entrenado
print(model_tree_alternative)
...
metricas_df <- data.frame(
  Modelo = c(rep("Random Forest", 3), rep("Decision Tree", 3), rep("LightGBM", 3)),
  Metrica = rep(c("RMSE", "MSE", "MAE"), 3),
  Valor = c(
    sqrt(mean((predicciones_rf - datos_prueba$Nota.media)^2)),
    mean((predicciones_rf - datos_prueba$Nota.media)^2),
    mean(abs(predicciones_rf - datos_prueba$Nota.media)),
    sqrt(mean((predicciones_prueba_tree - datos_prueba$Nota.media)^2)),
    mean((predicciones_prueba_tree - datos_prueba$Nota.media)^2),
    mean(abs(predicciones_prueba_tree - datos_prueba$Nota.media)),
    sqrt(mean((predicciones_prueba_lgbm - label_prueba)^2)),
    mean((predicciones_prueba_lgbm - label_prueba)^2),
    mean(abs(predicciones_prueba_lgbm - label_prueba))
  )
)

# Crear la gráfica
ggplot(metricas_df, aes(x = Modelo, y = Valor, fill = Metrica)) +
...
# Entrenamiento de los modelos con 80/20
