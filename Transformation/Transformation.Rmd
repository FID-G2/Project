...
datos_transf <- crear_interaccion(datos_transf, c("Sexo", "Ámbito.de.estudio","Rama.de.enseñanza"), Interaccion_Rama_Ambito_Sexo, TRUE)
```
...
columnas_numericas <- datos_transf[, c("Nota.media", "Número.de.estudiantes", "Diferencia_Nota_Media")]
```
...
```{r}
datos_transf[,  c("Nota.media", "Número.de.estudiantes", "Diferencia_Nota_Media")] <- datos_normalizados
```
...
datos_transf[, c("Nota.media", "Número.de.estudiantes", "Diferencia_Nota_Media")] <- datos_escalados
```
...
df_genero <- binary_encode(df_carrera_encoded, "Sexo", "Hombres", "Mujeres")
```
